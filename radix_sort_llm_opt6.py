...
@cuda.jit
def counting_sort_kernel(arr, output, exp, size):
    ...
    if tid < size:
        ...
        val = (arr[tid] // exp) % 10
        cuda.syncthreads()
        pos = shared_count[val] - 1
        cuda.atomic.sub(shared_count, val, 1)
...
def radix_sort_gpu(arr):
    ...
    exp = 1
    while max_num // exp > 0:
        counting_sort_kernel[blocks_per_grid, threads_per_block](d_arr, d_output, exp, size)
